...
## El gate rechaza mis `--results` con «lacks trusted Claude invocation identity» — ¿qué pasa?

Dos causas distintas daban el mismo mensaje, y el 2026-09-21 se vieron las dos el mismo día en cuatro sesiones vivas de un proyecto real. La primera era del toolkit: desde la 0.54.1 el gate identifica cada resultado por `planned_reviewer_id` (qué `Agent` lanzaste tú para ese hueco, no lo que el revisor dice ser), pero el ejemplo escrito en la skill de run conservó la forma anterior (`reviewer_id` arriba, sin `planned_reviewer_id`); quien lo copió tal cual fue rechazado. La segunda es de orquestación: el fork de review (o el panel de run) lanza los revisores en background y termina el turno "esperando"; en un fork o en una sesión headless no hay turno siguiente, nadie recoge los sobres, y el `--results` que se reconstruye después no tiene identidades reales. Desde la 0.54.3 el ejemplo de run coincide con `--plan`, el gate nombra cada causa por separado (`reviewer_id` sin `planned_reviewer_id` es un error propio, no el genérico), y la regla de foreground — con su señal: un id de tarea en vez del JSON del revisor — está escrita pegada a la llamada `Agent` en run y review, no solo en la regla 11 compartida. Si te pasa: relanza ese revisor en foreground en el mismo turno; nunca esperes la notificación.
...
## ¿Por qué el plugin ya no trae el hook de rtk?

Porque se midió y salía a pagar. Hasta la 0.44 `hooks/hooks.json` reescribía cada comando Bash a través de [rtk](https://www.rtk-ai.app), que promete un 60-90 % menos de tokens comprimiendo la salida de los comandos. JetBrains lo puso a prueba en julio de 2026 con 425 ejecuciones pareadas (mismo agente, mismas tareas, con y sin hook): con esfuerzo bajo el coste por tarea subió una mediana del **7,6 %** (p=0,004) y los turnos un 13,8 %; con esfuerzo alto, ±0. La calidad no cambió en ningún caso. La razón es estructural, no un bug: `Read`, `Grep` y `Glob` no pasan por el hook; la mitad de lo que corre en Bash (`python3 …`, pipes, heredocs) rtk no lo reescribe a propósito; y el grueso del coste de una sesión es releer contexto ya cacheado a una décima del precio, cosa que la compresión de una salida no toca. El techo teórico de lo que rtk puede tocar es ~3 % del input, y el propio rtk lo reconoce en su réplica. Su contador `rtk gain` mide contra la salida bruta completa, que Claude Code habría truncado igualmente, por eso enseña millones de tokens "ahorrados" mientras la factura sube. En este flujo, además, la mayor parte de la salida de comandos ya muere en los subagentes de `run` sin volver al hilo principal, así que había aún menos que comprimir. Un hook que ejecuta `jq` y `rtk rewrite` en cada llamada Bash a cambio de un efecto del orden del ruido no justifica ir de serie. Si quieres comprobarlo en tu propio corpus, la única medida válida es sesiones pareadas con y sin hook comparando la factura (`usage-sync.py report`), no el contador de la herramienta. Fuentes: [benchmark de JetBrains](https://blog.jetbrains.com/ai/2026/07/rtk-claude-code-token-savings/) y [réplica de rtk](https://www.rtk-ai.app/blog/rtk-on-skillsbench/).
