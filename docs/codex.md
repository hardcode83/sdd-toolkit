...
| `review`, `auto`, `diagram` | `review` and `auto` supported through the shared fail-closed panel; `diagram` unchanged | Native Codex panel uses installed reviewer-panel resources. Auto's headless delegation (`scripts/sdd_auto_outcome.py run`, a fresh `claude -p` per feature/review) is Claude-only: under Codex the script reports `AUTO_OUTCOME: UNAVAILABLE` and auto runs the phase inline, which is the documented fallback. |
