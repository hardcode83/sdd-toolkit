...
| Worktree isolation | Partially supported | `scripts/sdd_session.py` is Python 3 stdlib and works unchanged, so `check` / `claim` / `resolve` / `orphans` all answer correctly. What is missing is the `EnterWorktree` tool: nothing can switch the session's working directory, so isolation is **manual** — `git worktree add .claude/worktrees/sdd/<feature>`, then run the phase from that directory and `claim` it from there. Shared rule 10's branch guard in `run` still applies and still protects the merge evidence. |
