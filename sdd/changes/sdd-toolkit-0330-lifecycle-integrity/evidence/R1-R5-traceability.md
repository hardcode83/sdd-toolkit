...
|---|---|---|---|---|
| R1 stable anchor | D1; 1.1–1.3; 2.1–2.3; 4.1 | `scripts/sdd_lifecycle.py`: `lifecycle_commit`, `mark_ready`, `record_pr`, `classify_lifecycle_commit` | `pytest -q tests/test_sdd_lifecycle.py -k 'mark_ready_commits or record_pr_commits or lifecycle'` | Lifecycle child commits preserve the implementation anchor and contain no self-SHA. |
...
| R3 ship/record contract | D3/D4; 3.1–3.5; 4.1–4.4 | `validate_ship_suffix` CLI; `skills/ship/SKILL.md`; `record_pr` | `pytest -q tests/test_sdd_lifecycle.py tests/test_lifecycle_contract.py` | Every post-anchor commit is classified; record-pr creates STATE-only metadata and does not invoke push. |
...
- Focused lifecycle suite during implementation: exit `0`, 65 passed, 37 subtests passed.
