...
2. **Write** `sdd/changes/<feature>/tasks.md` using `${CLAUDE_PLUGIN_ROOT}/templates/tasks-template.md` — but first apply shared rule 6 (never silently overwrite an existing document; if `tasks.md` already exists, show it and ask regenerate/amend/keep — extra caution if any task is already `[x]`, since run may already be underway). Rules for the content:
   ...
   - **Section size is a cost**: `/sdd:run` delegates each section to a fresh implementer and runs the review panel at its close, so a section is the unit of both context and review. Prefer 4–8 sections that each leave the system working over 13 sections of two tasks; merge sections that would be reviewed together anyway.
   - **Mark the hard sections**: append `<!-- hard -->` to the heading of a section whose tasks have real solution variance (algorithms, state machines, tricky concurrency). `/sdd:run` implements those on the stronger model and the rest on the cheaper one; the marker is the only thing that escalates the model, so use it deliberately and say why at the gate.
   - **Mark the manual tasks**: append `<!-- manual -->` to a task only a human, or an environment the flow cannot reach from the worktree, can perform — a browser pass that needs a running stack, a console step, an external system. `/sdd:run` leaves them to the human, and `mark-local-verified` lets them travel with the PR as a `deferred` entry instead of blocking it; `/sdd:archive` still requires them done (ADR 0006). Everything that can be verified with a command is not manual.
   - **Implementation Notes**: keep the template's closing `## Implementation Notes` section (empty). Implementers append to it what the next section needs — decisions, names, gotchas — so a fresh implementer per section loses nothing.
