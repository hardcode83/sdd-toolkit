...
1. **Verify completion.** Mark the phase for usage attribution: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/usage-mark.sh" <feature> archive` (run it unconditionally — the script itself no-ops when tracking is off; NEVER skip it based on your own assessment of whether metrics are enabled). Read the change's `tasks.md`. If any task is unchecked, list them and ask the user to confirm archiving anyway (they may have been done outside the flow) — never check boxes yourself here.
