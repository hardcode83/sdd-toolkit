...
1. **Load context.** Read `sdd/project.md` and skim `sdd/specs/` for capabilities this change touches. If `sdd/` doesn't exist, tell the user to run `/sdd:init` first and stop. Mark the phase for usage attribution: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/usage-mark.sh" <feature> new` (run it unconditionally — the script itself no-ops when tracking is off; NEVER skip it based on your own assessment of whether metrics are enabled).
   ...
   - **Ad-hoc features — no blind changes**: if the feature does NOT come from a roadmap entry and `sdd/roadmap.md` exists, ask the user (AskUserQuestion; recommend yes) whether to register it as an ad-hoc entry: inserted where it belongs in the execution order (usually right after the entry it grows out of), with a provenance note in the style the roadmap already uses (e.g. *"(no está en el plan original, añadido tras `<entry>`)"*) and the ` → changes/<feature>/` annotation. If the user declines, say explicitly that the change will live outside the roadmap's picture (status/archive won't track it there). Never add it silently — roadmap rewrites must never be a surprise.
