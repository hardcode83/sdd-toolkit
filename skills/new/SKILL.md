...
1. **Load context.** Read `sdd/project.md` and skim `sdd/specs/` for capabilities this change touches. If `sdd/` doesn't exist, tell the user to run `/sdd:init` first and stop. Mark the phase for usage attribution: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/usage-mark.sh" <feature> new` (run it unconditionally — the script itself no-ops when tracking is off; NEVER skip it based on your own assessment of whether metrics are enabled).
   - **Isolation check** (shared rule 10, before creating any branch): `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/sdd_session.py" --root . check --feature <feature>`. On `CLEAR`, continue here. On `CONFLICT`, another session is working this clone (or HEAD sits on another feature's branch, or this clone holds other in-flight changes) — **offer** the worktree with AskUserQuestion, recommending yes, and name the evidence the check reported. Then follow `${CLAUDE_PLUGIN_ROOT}/references/isolation.md`: base-ref check → `EnterWorktree` → bootstrap from `sdd/project.md` → `sdd_session.py … claim <feature>`. If the user declines, say plainly what can break (branch drift and, through it, false `head_branch`/`implementation_sha` in `STATE.md`) and continue where you are — it is their call. On `CLEAR`, still `claim` the feature so the next session sees it.
