...
name: new
model: opus
description: Start a new SDD change - creates sdd/changes/<feature>/proposal.md with the why, scope, and EARS requirements. Use when the user runs /sdd-toolkit:new or asks to spec out a feature.
---
...
2. **Understand the ask.** If the request is ambiguous on something that changes the requirements (not implementation details), ask the user 1-3 targeted questions (AskUserQuestion). Otherwise proceed.
   - **Adopting in-flight work**: if the feature is already partially built (the user says so, or the code clearly shows it), the proposal documents the *intended end state* as usual — reality is captured later, when `/sdd-toolkit:tasks` pre-checks what's already done.
...
5. **Gate.** Present a short summary and ask the user to review. On approval, suggest the next step: `/sdd-toolkit:design` — or `/sdd-toolkit:tasks` directly if the change is trivial (no new architecture, few files, obvious approach).
