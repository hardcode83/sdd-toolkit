...
**UI/UX and design-system lens (specialized offering).** If step 2 detected a frontend/design-system surface, name this lens among the offers above — do not impose it. It verifies *consistency* with the project's own tokens/components and essential states (twelve objective areas plus an observable-evidence-only list of generic AI-UI patterns), never redesigning or proposing visuals. Unlike the generic lenses above, it ships pre-filled: if accepted,

- Generate `.claude/agents/sdd-review-ui-ux.md` from `${CLAUDE_PLUGIN_ROOT}/templates/reviewer-ui-ux.md` — not the generic `reviewer-template.md`; this template already carries its twelve areas, its AI-tells list, and the panel's JSON output contract.
- Guarantee the frontend/design-system steering baseline the lens cites as its referent: if `sdd/steering/frontend.md` doesn't exist, create it from `${CLAUDE_PLUGIN_ROOT}/templates/steering/frontend.md` (its *Design system* section is where this project's own aesthetic authority goes — never a third-party design language). If it already exists, do **not** overwrite it — but its mere existence is not the guarantee: check that it actually carries the objective UI/UX baseline (contrast, focus visible, keyboard operability, interaction target size, essential states, responsive) that the template's *Accessibility* and *States*/*Responsive* sections define. If that baseline is missing entirely or only partially, complete it **additively** — add the missing items without touching, reordering, or weakening the project's existing rules.
- Derive `applies_to` from the frontend root(s) step 2 already detected — not the template's example globs, and not a guessed broad pattern. If those roots cannot be determined reliably, don't guess: use AskUserQuestion to expose the ambiguity and let the user name the visual-surface paths, then use that answer.
- When this lens comes up alongside the `frontend-design` plugin (offered in step 6), reaffirm the overlap already documented in `${CLAUDE_PLUGIN_ROOT}/references/plugin-catalog.md:34-37`: `frontend-design` pushes the agent toward a *distinctive* look when writing new UI; this lens verifies *consistency* with what the project already has. `sdd/steering/frontend.md` wins over either's taste. Offering this lens never makes `frontend-design` a dependency, and accepting one is not a prerequisite for the other.
...
1. **MCPs** (multiSelect) — read `${CLAUDE_PLUGIN_ROOT}/references/mcp-catalog.md`; offer only the entries relevant to the detected stack (e.g. don't offer Postgres to a project with no database; for a web frontend offer the *one* browser tool that matches the job the catalog's table describes, not all three).
...
6. **Official plugins and skills** (multiSelect) — read `${CLAUDE_PLUGIN_ROOT}/references/plugin-catalog.md`; offer the entries relevant to the detected stack/team, watching the overlap rules it documents (LSPs live in their own catalog; integrations must not be offered both as plugin and raw MCP; a frontend gets `frontend-design` *and* the reminder that its steering doc wins over the skill's taste). Never offer the entries in its "Not offered" section, however well known — they are rival methodologies or duplicates of the panel, and the reasons are written there for when the user asks. You cannot install plugins yourself — print the exact `/plugin install <name>@claude-plugins-official` commands for the user, and point them to the `/plugin` Discover tab as the browsable catalog.
...
- **MCPs**: merge the chosen entries into the project's `.mcp.json`. If the file exists, preserve every existing server — only add new keys. Mention any auth step the catalog notes.
  **Offer only what this project needs, and say what each thing costs.** The fixed cost per request used to be the tool schemas: a median of 42k tokens before a session did anything, measured on 19k requests. Since Claude Code 2.1.215 MCP schemas are discovered on demand (Tool Search), so an idle server is nearly free — what still lands on every request is *hooks* (whatever they inject, on every matching event), *skill descriptions* (one line each, always), MCP *instructions*, and above all tool *outputs* once a server is used. So: a hook or an always-on skill needs a stronger reason than an MCP server does; and a server still belongs in the project's `.mcp.json`, not in the global config — one that belongs to another of the user's projects is approval prompts and discovery noise here. Say so when the global config carries ones this stack has no use for, and point at per-project `.mcp.json` and `.claude/settings.json` as where that gets scoped (shared rule 11).
