...
2. **Delegate each section to a fresh implementer.** For every section in scope with unchecked tasks, in order, launch **one** `Agent` (general-purpose) and wait for it. The implementer starts with an empty context, so the prompt must be complete:
   ...
   **Model — always explicit, always an alias.** Every `Agent` call carries `model:` (`${CLAUDE_PLUGIN_ROOT}/references/models.md`): `sonnet` for a section by default; `opus` for a section whose heading carries `<!-- hard -->` — set by `/sdd:tasks` or by the user for algorithms, state machines, tricky concurrency. A call without `model` inherits the session model, so in an Opus session every implementer and every fix round would silently run on Opus (measured on the first real auto run, whose calls carried none). Aliases, never model IDs: the environment maps `haiku`/`sonnet`/`opus`/`fable` to the provider's models (MiniMax, a gateway, Codex — see the reference), and a model ID written here would break that. Never escalate on your own initiative; a section that turns out harder than planned is a note for the user at the gate, not a silent model change.

   **`<!-- manual -->` tasks** (a browser pass, a console step, an external system) are not the implementer's: leave them unchecked and untouched, tell the implementer so, and handle them in step 6.
...
3. **Review panel — after each completed section.** When the last task of a numbered section is checked and the section touched production code (skip it for pure scaffolding/docs/config sections, and in `solo` mode), launch the review panel **in parallel** (one message, one `Agent` call per reviewer):
   ...
   **Results are JSON, not reports.** Each reviewer's final message is the result
   ...
   - **Fix rounds are delegated too, and they climb a ladder.** Give the accepted findings (file:line, referent, what, fix direction) to a fresh implementer with the same contract as step 2, scoped to those findings; then re-run **only the reviewer(s) whose findings were fixed**, scoped to the fix.
     ...
     - **Round 2**: implementer on `model: opus`, given the findings **and the round-1 implementer's report** — what it changed and, if it disagreed, why. The stronger model is bought for the case where the first fix missed the point, not for typing faster.
     - **After round 2, read what persists before doing anything.** *Persistence* (the same finding, same referent, same location, with the implementer stating the finding is wrong) is a disagreement between reviewer and implementer: stop and present **both** positions to the user — the finding with its referent quoted, the implementer's reasoning, the diff — as a `decision` (shared rule 5). *Churn* (the reviewer raises new findings each round in the same area) is a steering rule or a D# too vague to converge on: stop, name the pattern and the rule, and hand it to the user as a `decision` too. Two rounds is the cap in both cases; a third round of edits fixes neither.
