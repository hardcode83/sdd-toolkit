...
name: status
model: haiku
description: Show the state of SDD changes - active changes, phase, task progress, and the roadmap as a to-do view. Use when the user runs /sdd-toolkit:status or asks where a change or the roadmap stands.
---
...
4. Present a compact table: change · phase · tasks done/total · suggested next command (`/sdd-toolkit:design`, `/sdd-toolkit:tasks`, `/sdd-toolkit:run`, or `/sdd-toolkit:archive`). Below it, the roadmap to-do view with a progress count (e.g. `2/13`).

If `sdd/` doesn't exist, say so and point to `/sdd-toolkit:init`. If there are no active changes, say so and point to `/sdd-toolkit:new` (suggesting the next roadmap entry if there is one).
