...
class ReviewerParityTests(unittest.TestCase):
    ...
    def test_provider_requests_are_derived_from_registry_plan(self):
        ...
        plan = self.rp.build_reviewer_plan(ROOT, "review", scope)
        expected = [d.reviewer_id for d in self.rp.load_registry()]
        refs = {"requirements": "R1", "design": "D1", "steering": "read-only", "scope": "src/a.py"}
        claude = FakeClaude([{"reviewer_id": item.reviewer_id, "scope_id": item.scope_id, "verdict": "PASS", "findings": [], "evidence": ["src/a.py"], "status": "complete"} for item in plan])
        codex = FakeCodex(claude.payloads)
        self.rp.dispatch_claude_panel(plan, claude, "x", refs)
        self.rp.dispatch_codex_panel(plan, codex, "x", ROOT, refs)
        self.assertEqual([p.reviewer_id for p in plan[:3]], expected)
