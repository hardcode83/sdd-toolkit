...
| `/sdd:archive [feature]` | Empieza por `preflight-archive`, un comando que comprueba todas las precondiciones a la vez con su arreglo exacto (31 de 64 archives fallaban a la primera). Exige PR mergeado verificable; después fusiona specs vivas, consolida métricas, finaliza roadmap y archiva. | haiku |
...
| `/sdd:review [feature]` | Con feature: valida localmente, deja el **recibo del panel** (veredicto por revisor y HEAD juzgado, en el directorio git común) y, si pasa, registra `READY_FOR_PR` en el mismo turno y ofrece publicar con `/sdd:ship`. Un re-review tras fixes relanza solo lo que no pasó (`--carry` cuando solo cambiaron documentos). Sin argumento: enumera los changes activos **de todos los worktrees** y pregunta; si no hay ninguno, drift specs↔código. | sonnet |
| `/sdd:ship [feature]` | Publica un change en `READY_FOR_PR` (registra él mismo los hitos si el recibo del panel certifica HEAD, y hace él mismo el push inicial de la rama si nadie la ha publicado, en vez de devolverte comandos): sincroniza la base en la rama (`sync-base`, merge nunca rebase), push, PR y evidencia registrada (`record-pr`). Resuelve solo el bookkeeping append-only; un conflicto de código lo resuelve y **vuelve a verificar** con el comando del proyecto antes de publicar. No revisa, no mergea el PR, no archiva. Sin remoto o sin `gh`, entrega la acción manual exacta en vez de fallar. | sonnet |
...
| `SDD032` | warning | Una sección lleva `panel: PASS` sin `receipt:<id>` o con un recibo que no existe o no coincide: la anotación no la escribió el gate, review la trata como no revisada. |
...
**Si tu herramienta ya te dio un worktree (Orca, `claude -w`), ese worktree es el aislamiento.** El check de sesión describe *este directorio*, no el clon entero: otra sesión viva en otro worktree no comparte tu HEAD y no es conflicto, y `isolation: always` queda satisfecha por el worktree en el que estás — `check` dice `WORK HERE` y avisa si la feature ya tiene otro worktree. Antes contaba a todas las sesiones del registro compartido, y un worktree limpio de Orca se creaba un segundo worktree al lado: dos stacks de Docker por feature, cinco features, 192 MB libres ([ADR 0007](docs/adr/0007-panel-receipt-and-directory-check.md)).
...
- **MCPs** (`references/mcp-catalog.md`): GitHub, Atlassian, navegador (Playwright / Chrome DevTools / `claude --chrome`, uno según el trabajo), Context7, Postgres, Sentry, Linear, Figma… → `.mcp.json` (merge). Con Tool Search un servidor en reposo casi no cuesta; el catálogo dice qué cuesta cada uno *al usarlo*.
- **LSPs** (`references/lsp-catalog.md`): doce plugins LSP oficiales (Python, TS, Rust, Go, Java, Kotlin, C#, C/C++, PHP, Ruby, Swift, Lua); instala binarios con aprobación e imprime los `/plugin install`. Serena solo para bases grandes.
- **Plugins y skills oficiales** (`references/plugin-catalog.md`): sugerencias curadas del marketplace `claude-plugins-official` según el stack — `frontend-design`, `webapp-testing`, `hookify`, `security-guidance`, `pr-review-toolkit`… — con reglas anti-solape (nunca ofrecer una integración como plugin Y como MCP crudo) y una sección **"no ofrecemos"** con los porqués: `superpowers`, `feature-dev` y `ralph-loop` son metodologías rivales que pisan las fases; `code-review` duplica el panel. El marketplace oficial no es consultable programáticamente — el catálogo curado es la fuente y tú ejecutas los `/plugin install`; la pestaña Discover de `/plugin` es el complemento navegable.
